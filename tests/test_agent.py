...
def test_llm_sql_with_unknown_set_columns_falls_back(tmp_path, monkeypatch):
    schema = tmp_path / "schema.yaml"
    schema.write_text(
        """
tables:
...
""".strip()
    ...
    assert "UPDATE stg_orders" in rec.sql
    assert "fallback template" in rec.explanation.lower()
...
""".strip()
    ...
    class FakePrompt:
        ...
        def invoke(self, payload):
            ...
            if "candidates_json" in payload:
                return json.dumps({"table": "stg_orders", "target_status": "READY_RETRY", "reason": "orders"})
            return json.dumps(
                {
                    "sql": "UPDATE stg_orders SET record_status = :restart_status WHERE order_id = :pk_value AND record_status = :failed_status;",
                    "params": {"restart_status": "READY_RETRY"},
                    "explanation": "Generated by fake llm",
                }
            )
...
def test_from_log_can_infer_schema_and_generate_sql_with_llm(monkeypatch):
    ...
    class FakePrompt:
        ...
        def invoke(self, payload):
            if "schema_json" not in payload and "candidates_json" not in payload and "pk_column" not in payload:
                return json.dumps(
                    {
                        "table": "stg_orders",
                        "primary_keys": ["order_id"],
