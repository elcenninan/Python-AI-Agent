...
class SQLUpdateAgent:
    ...
    def _has_required_safety_guards(
        ...
        pk_pattern = rf"\b{re.escape(pk_column.lower())}\b\s*=\s*(:[a-zA-Z_][a-zA-Z0-9_]*|'[^']*'|\"[^\"]*\")"
        has_pk_guard = bool(re.search(pk_pattern, where_clause_lower))
        if not has_pk_guard:
            return False

        status_pattern = rf"\b{re.escape(status_column.lower())}\b\s*=\s*(:[a-zA-Z_][a-zA-Z0-9_]*|'[^']*'|\"[^\"]*\")"
        has_status_guard = bool(re.search(status_pattern, where_clause_lower))
        if not has_status_guard:
            return False
        ...
        return has_pk_guard and has_status_guard
    ...
    def _build_llm_recommendation(
        ...
        has_required_guards = self._has_required_safety_guards(
            sql=sql,
            params=params,
            pk_column=pk_column,
            pk_value=pk_value,
        ...
        if not has_required_guards:
            return None, (
                "LLM SQL did not include required safety guards "
                "(WHERE must constrain both primary key and failed-status rows)."
            )
